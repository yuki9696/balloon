...
class AudioPlayer:NSObject {
    ...
    var didFinishPlaying:((_ finished:Bool) -> Void)?
    var currentIndex:Int = -1
    var mode:AudioPlayerMode = AudioPlayerMode.stopped {
        didSet{
            if mode == .stopped {
                self.player.stop()
                self.player = nil
    ...
    //-------- LOGIC:--->
    // the logic behind this 'object', 
    // well, it is depends on key 'chats', when user tap on
...
extension AudioPlayer {
    ...
    func next() -> Bool {
        ...
        if nextIndex < count {
            ...
            print("didStopPlaying LastIndex: \(idx)")
            self.didStopPlaying?(idx)
